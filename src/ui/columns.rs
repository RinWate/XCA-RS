...
/// One factory for both flat and tree columns: `tree` selects the
/// two-level property chain through `TreeListRow`, `expander` wraps the
/// cell in a `GtkTreeExpander` (indent + ▸/▾, like the original XCA).
fn make_column(
    title: &str,
    prop: &'static str,
    expand: bool,
    ...
    factory.connect_setup(move |_, item| {
        ...
        if let Some(slot) = &menu {
            ...
            gesture.connect_pressed(move |_, _, x, y| {
                ...
                let obj = li.item().and_then(|o| {
                    if tree {
                        o.downcast::<gtk::TreeListRow>()
                            .ok()
                            .and_then(|r| r.item())
                    } else {
                        Some(o)
...
pub fn text_column(
    ...
    make_column(title, prop, expand, menu, false, false)
}

/// A column over a `TreeListModel` (no passthrough): the list item is a
/// `TreeListRow`, so the property is resolved through its `item`.
pub fn tree_column(
    title: &str,
    prop: &'static str,
    expand: bool,
    ...
    make_column(title, prop, expand, menu, true, expander)
}
